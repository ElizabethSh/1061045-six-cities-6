...
describe(`Reducer 'user' should work correctly`, () => {
  ...
  it(`Reducer should update isLoggedIn to 'true'`, () => {
    ...
    expect(user(state, setAuthStatusAction)).toEqual({
      isAuthChecked: false,
      usersEmail: null,
      isLoggedIn: true,
    });
  ...
  it(`Reducer should update users email to given value`, () => {
    ...
    expect(user(state, setUsersEmailAction)).toEqual({
      isAuthChecked: false,
      usersEmail: `test@test.com`,
      isLoggedIn: false,
    });
  });
  ...
  it(`Reducer should update isAuthChecked to 'true'`, () => {
    ...
    expect(user(state, checkAuthAction)).toEqual({
      isAuthChecked: true,
      usersEmail: null,
      isLoggedIn: false,
    });
...
describe(`Async operation work correctly`, () => {
  it(`Should make a correct API call (GET) to /login`, () => {
    ...
    apiMock.onGet(APIRoute.LOGIN).reply(200, { email: `test@gmail.com` });
  ...
  it(`Should make a correct API call (POST) to /login`, () => {
    ...
    const logInLoader = logInAction(fakeUser);
  ...
  it(`Should make a correct API call (GET) to /logout`, () => {
    const apiMock = new MockAdapter(api);
    const dispatch = jest.fn();
    const logOutLoader = logOutAction();

    apiMock.onGet(APIRoute.LOGOUT).reply(200, [{ fake: true }]);
